...
import { splashTranslations, isRTL } from '@/src/shared/core/constants/translation';
import {
    horizontalScale,
    verticalScale,
    ScaleFontSize,
} from '@/src/shared/core/utils/scaling';
import { useAuthState, getRoutePath } from '@/src/shared/hooks/auth';
...
export default function SplashScreenComponent() {
    ...
    const { isHydrated, getSplashDestination } = useAuthState();
    ...
    const navigateToScreen = useCallback(() => {
        if (!isHydrated) {
            ...
            setTimeout(navigateToScreen, 100);
            return;
        }

        const { destination, reason } = getSplashDestination;
        console.log(`[SplashScreen] Navigating to ${destination}: ${reason}`);

        const route = getRoutePath(destination);
        ...
        try {
            ...
            router.replace('/OnBoardingScreen' as never);
        }
    }, [isHydrated, getSplashDestination, router]);
